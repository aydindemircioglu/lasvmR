...
test_that("KMeans trains as expected on synthetical dataset", {
	...
	#print(sum(x))
	E = lasvmR::orthoKMeansTrain (x = x, k = 8, rounds = 4, verbose = FALSE)
...
#	dump("E", "/tmp/A")
	...
	E = lasvmR::orthoKMeansTrain(x, k = k, rounds = 2, verbose = FALSE)
...
test_that("KMeans predicts as expected on synthetical dataset", {
	...
	k <- 8
	x <- matrix(rnorm(25000, mean=0.5, sd=1), ncol=5)
	obj = lasvmR::orthoKMeansTrain (x = x, k = 8, rounds = 4, verbose = FALSE)
	...
	set.seed(100)
	x <- matrix(rnorm(25000, mean=0.5, sd=1), ncol=5)
	E = lasvmR::orthoKMeansPredict (x = x, obj = obj, verbose = FALSE)
